...
import { Bot, Users, Wifi, Zap, Trophy, Shield } from 'lucide-react';
...
const GameModeSelection: React.FC<GameModeSelectionProps> = ({ onSelectMode }) => {
  const gameModes = [
    ...
    },
    {
      ...
      description: 'Online multiplayer with a friend',
      icon: Wifi,
      color: 'from-green-500 to-emerald-500',
      features: ['Online Multiplayer', 'Real-time Play', 'Cross-Device']
    }
  ];
          ...
          {gameModes.map((gameMode, index) => (
            ...
            >
              <motion.div
                ...
                className={`bg-game-card border border-game-border rounded-2xl p-8 cursor-pointer transition-all duration-300 hover:shadow-2xl hover:shadow-purple-500/20 relative overflow-hidden`}
              >
                {/* Background Gradient */}
                <div className={`absolute inset-0 bg-gradient-to-br ${gameMode.color} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
                
                {/* Content */}
                <div className="relative z-10">
                  {/* Icon */}
                  <div className="flex justify-center mb-6">
                    <div className={`p-4 rounded-2xl bg-gradient-to-br ${gameMode.color} shadow-lg`}>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`w-full mt-8 py-4 px-6 rounded-xl bg-gradient-to-r ${gameMode.color} text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-300`}
                  >
                    Play Now
                  </motion.button>
                </div>
